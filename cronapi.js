...
(function () {
  ...
  this.cronapi.evalInContext = function (js) {
    let result = eval('this.cronapi.doEval(' + js + ')');
    if (result) {
      if (result.commands) {
        for (let i = 0; i < result.commands.length; i += 1) {
          let func = eval(result.commands[i].function);
  ...
  this.cronapi.server = function (pack) {
    ...
    return {
      ...
      run: function () {
        ...
        let blocklyName = namespace + ":" + func;
        ...
        let args = [blocklyName, success, error];
  ...
  this.cronapi.conversion.asciiToBinary = function (astring) {
    ...
    if (astring.length > 0) {
      for (let i = 0; i < astring.length; i += 1) {
        let value = astring.charCodeAt(i);
        for (let j = 7; j >= 0; j -= 1) {
          binary += ((value >> j) & 1);
        }
      }
  ...
  this.cronapi.conversion.stringToDate = function (value) {
    let pattern = /^\s*(\d+)[\/\.-](\d+)[\/\.-](\d+)(\s(\d+):(\d+):(\d+))?\s*$/;
    if (value) {
      if (value instanceof Date) return value;
      else if (pattern.test(value)) {
        let splited = pattern.exec(value);
        let userLang = (navigator.language || navigator.userLanguage).split('-')[0];
        if (userLang === 'pt' || userLang === 'en') {
          let functionToCall = eval('cronapi.internal.' + userLang + 'Date');
  ...
  this.cronapi.util.callServerBlocklyAsync = function (classNameWithMethod, fields, callbackSuccess, callbackError) {
    let serverUrl = 'api/cronapi/call/body/#classNameWithMethod#/'.replace('#classNameWithMethod#', classNameWithMethod);
    let http = this.cronapi.$scope.http;
    let params = [];
    ...
    let finalUrl = this.cronapi.internal.getAddressWithHostApp(serverUrl);
  ...
  this.cronapi.util.getScreenFields = function () {
    ...
    for (let key in this.cronapi.$scope) {
    ...
    let recursiveLookup = function (scope) {
      let fieldValue;
      try {
        fieldValue = eval(scope.lets);
      }
      catch (e) {
      }
    ...
    for (let key in this.cronapi.$scope.params) {
  ...
  this.cronapi.util.makeCallServerBlocklyAsync = function (blocklyWithFunction, callbackSuccess, callbackError) {
    ...
    paramsApply.push(function (data, status, errorThrown) {
      let message = this.cronapi.internal.getErrorMessage(data.responseText, errorThrown);
  ...
  this.cronapi.util.language = function () {
    let locale = (window.navigator.userLanguage || window.navigator.language || 'pt_br').replace('-', '_');
    return locale;
  };
  ...
  this.cronapi.util.callServerBlockly = function (classNameWithMethod) {
    ...
    if (resultData.status === 200) {
      ...
      let message = this.cronapi.internal.getErrorMessage(resultData.responseText, resultData.statusText);
      this.cronapi.$scope.Notification.error(message);
      throw message;
    }
    return result;
  ...
  this.cronapi.util.callServerBlocklyAsynchronous = function (classNameWithMethod, callback, params) {
    if (classNameWithMethod != '' && typeof callback === 'function') {
      ...
      params.push(callback);
      let idx = 2;
      for (idx; idx < arguments.length; idx += 1) {
  ...
  this.cronapi.util.downloadFile = function (url) {
    ...
    let finalUrl = this.cronapi.internal.getAddressWithHostApp(url);
  ...
  this.cronapi.util.getURLFromOthers = function (/** @type {ObjectType.STRING} @description {{HTTPMethod}} @blockType util_dropdown @keys GET|POST|PUT|DELETE|PATCH|HEAD|OPTIONS|TRACE @values GET|POST|PUT|DELETE|PATCH|HEAD|OPTIONS|TRACE  */  method, /** @type {ObjectType.STRING} @description {{HTTPMethod}} @blockType util_dropdown @keys application/x-www-form-urlencoded|application/json @values application/x-www-form-urlencoded|application/json  */  contentType, /** @type {ObjectType.STRING} @description {{URLAddress}} */ url, /** @type {ObjectType.OBJECT} @description {{paramsHTTP}} */ params, /** @type {ObjectType.OBJECT} @description {{headers}} */ headers, /** @type {ObjectType.STATEMENTSENDER} @description {{success}} */ success, /** @type {ObjectType.STATEMENTSENDER} @description {{error}} */  error) {
    ...
    let header = Object.create(headers);
  ...
  this.cronapi.screen.changeValueOfField = function (/** @type {ObjectType.STRING} @blockType field_from_screen*/ field, /** @type {ObjectType.STRING} */value) {
    try {
      ...
      let func = new Function('this.' + field + ' = this.__tempValue;');
  ...
  this.cronapi.screen.changeView = function (view, params) {
    try {
      ...
      let paramsStopEncode = {};
      paramsStopEncode['%24'] = '$';

      function decodeCharParam(value) {
        if (value) {
          for (let param in paramsStopEncode) {
            let regex = eval('/' + param + '/g');
      ...
      if (typeof params !== 'undefined') {
        for (let i in Object.keys(params)) {
          let k = Object.keys(params[i])[0];
          let v = String(Object.values(params[i])[0]);
          if (queryString) {
            queryString += "&";
          }
      ...
      let oldHashToCheck = oldHash + (oldHash.indexOf("?") > -1 ? '' : "?");
      let viewToCheck = view + (view.indexOf("?") > -1 ? '' : "?");
  ...
  this.cronapi.screen.hideIonicModal = function (/** @type {ObjectType.OBJECT} @blockType ids_from_screen*/ id) {
    if ($('#' + id).data('cronapp-modal')) {
      let modal = $('#' + id).data('cronapp-modal');
      modal.remove();
  ...
  this.cronapi.dateTime.getMomentObj = function (value) {
    ...
    if (value instanceof moment) {
      ...
      let formats = this.cronapi.dateTime.formats();
      let momentObj = null;
      for (let ix in formats) {
  ...
  this.cronapi.dateTime.getSecond = function (value) {
    let date = this.cronapi.dateTime.getMomentObj(value);
    if (date)
  ...
  this.cronapi.dateTime.getSecondsBetweenDates = function (date, date2) {
    let dateVar = this.cronapi.dateTime.getMomentObj(date);
    let date2Var = this.cronapi.dateTime.getMomentObj(date2);
  ...
  this.cronapi.dateTime.updateDate = function (value, year, month, day, hour, minute, second, millisecond) {
    let date = this.cronapi.dateTime.getMomentObj(value).toDate();
  ...
  this.cronapi.dateTime.updateNewDate = function (date, /** @type {ObjectType.STRING} @description {{attribute}} @blockType util_dropdown @keys year|month|day|hour|minute|second|millisecond  @values {{year}}|{{month}}|{{day}}|{{hour}}|{{minute}}|{{second}}|{{millisecond}}  */ type, value) {
    let updatedDate = this.cronapi.dateTime.getMomentObj(date).toDate();
...
*/
  ...
  this.cronapi.xml.newXMLEmptyWithRoot = function (rootElement) {
    let t__temp = $.parseXML('<?xml version="1.0" encoding="UTF-8"?><root></root>');
  ...
  this.cronapi.xml.newXMLElement = function (elementName, value) {
    let t__tempElement = document.createElement(elementName);
  ...
  this.cronapi.xml.renameElement = function (element, name) {
    let newElement = element.outerHTML.replace(element.localName, name)
  ...
  this.cronapi.i18n.translate = function (value, params) {
    if (value) {
      let text = this.cronapi.$translate.instant(value);
  ...
  this.cronapi.internal.startCamera = function (field) {
    ...
    if (window.cordova && window.cordova.platformId && window.cordova.platformId !== 'browser') {
      ...
      let cameraContainer = '<div class="camera-container" style="margin-left:-$marginleft$;margin-top:-$margintop$">\
      ...
      if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
        navigator.mediaDevices.getUserMedia(mediaConfig).then(function (stream) {
          ...
          let res = getMaxResolution(stream.getTracks()[0].getSettings().width, stream.getTracks()[0].getSettings().height);
          let halfWidth = res.width;
          let halfHeight = res.height;
          try {
          ...
          cameraContainer =
            ...
            ;
          let cronapiVideoCapture = $(cameraContainer);
          cronapiVideoCapture.prependTo('body');
          let videoDOM = document.getElementById('cronapiVideoCapture');
  ...
  this.cronapi.internal.downloadFileEntityMobile = function (datasource, field, indexData) {
    ...
    try {
      let tempJsonFileUploaded = JSON.parse(valueContent);
    }
    catch (e) { }

    if (tempJsonFileUploaded) {
      let finalUrl = this.cronapi.internal.getAddressWithHostApp('/api/cronapi/filePreview/');
      ...
      if (datasource.isOData()) {
        ...
        let url = urlCreator.createObjectURL(new Blob([bytesOrFileInput], { type: 'application/octet-stream' }));
        downloadUrl(url, fileName);
      }
      else {
        let url = '/api/cronapi/downloadFile';
        let splited = datasource.entity.split('/');

        let entity = splited[splited.length - 1];
        if (entity.indexOf(':') > -1) {
          //Siginifica que é relacionamento, pega a entidade do relacionamento
          let entityRelation = '';
          let splitedDomainBase = splited[3].split('.');
          for (let i = 0; i < splitedDomainBase.length - 1; i += 1)
            entityRelation += splitedDomainBase[i] + '.';
          let entityRelationSplited = entity.split(':');
          entity = entityRelation + entityRelationSplited[entityRelationSplited.length - 1];
        }
        url += '/' + entity;
        url += '/' + field;
        let object = itemActive;
        let ids = datasource.getKeyValues(object);
        let currentIdxId = 0;
        for (let attr in ids) {
          if (currentIdxId === 0)
            url = url + '/' + object[attr];
  ...
  this.cronapi.internal.captureFromCamera = function (field, width, height) {
    let canvas = document.createElement('canvas'); // create img tag
    canvas.width = width;
  ...
  this.cronapi.internal.generatePreviewDescriptionByte = function (data) {
    ...
    try {
      ...
      try {
        ...
        if (data && data.indexOf('dropboxusercontent') > -1) {
          json = {};
          let urlSplited = data.split('/');
          let fullName = urlSplited[urlSplited.length - 1].replace('?dl=0', '');
          ...
          let fileName = 'download';
          let fileExtesion = this.cronapi.internal.getExtensionBase64(data);
          let contentType = this.cronapi.internal.getContentTypeFromExtension(fileExtesion);
    ...
    if (json) {
      ...
      let result = "<b>Nome:</b> <br/>" + json.name + "<br/>";
    ...
    if (tempJsonFileUploaded) {
      ...
      if (datasource.isOData()) {
        ...
        this.$promise = this.cronapi.$scope.$http({
          ...
          headers = headers();
          let filename = headers['x-filename'] || 'download.bin';
          let urlCreator = window.URL || window.webkitURL || window.mozURL || window.msURL;
          try {
            let url = urlCreator.createObjectURL(data);
            downloadUrl(url, filename);
          } catch (ex) {
  ...
  this.cronapi.internal.uploadFileAjax = function (field, file, progressId) {
    let uploadUrl = '/api/cronapi/uploadFile';
    let formData = new FormData();
    formData.append("file", file);
    let _u = JSON.parse(localStorage.getItem('_u'));

    let finalUrl = this.cronapi.internal.getAddressWithHostApp(uploadUrl);
  ...
  this.cronapi.internal.uploadFile = (function fnUploadFile(field, file, progressId) {
    if (!file) return;

    const regexForDatasource = /(.*?).active./g;
    const groupDatasource = regexForDatasource.exec(field);
    // Verificar se é campo de um datasource
    if (groupDatasource) {
      const datasource = eval(groupDatasource[1]);
      if (datasource.isOData()) {
        const regexForField = /.active.([a-zA-Z0-9_-]*)/g;
        const groupField = regexForField.exec(field);
        const fieldName = groupField[1];
        const schemaField = datasource.getFieldSchema(fieldName);
   ...
   * @returns {ObjectType.OBJECT}
   */
  this.cronapi.object.getProperty = function fnGetProperty(object, property) {
    const splited = property.split('.');
    if (splited.length > 1) {
      const recursiva = function (object, params, idx) {
        if (!idx) { idx = 0; }
  ...
  this.cronapi.object.setProperty = function (object, property, value) {
    let splited = property.split('.');
    if (splited.length > 1) {
      const recursiva = (function (object, params, value, idx) {
        if (!idx) idx = 0;
        if (object[params[idx]] === undefined) object[params[idx]] = {};
        idx += 1;
        if (idx < params.length) {
  ...
  this.cronapi.cordova.file.createFile = function (dirEntry, fileName, content, success, error) {
    let path = (dirEntry || getDirectory(0));
   ...
   * @returns {ObjectType.VOID}
   */
  this.cronapi.cordova.database.openDatabase = function fnOpenDatabase(dbName) {
    if (!dbName) {
      dbName = this.cronapi.cordova.database.nameDefault;
    } 
   ...
   * @returns {ObjectType.VOID}
   */
  this.cronapi.cordova.database.executeSql = function fnSql(dbName, text, array, success, error) {
    // exist DB
    const db = this.cronapi.cordova.database.openDatabase(dbName); // create DB
    // open transaction
    db.transaction(function fnConnect(connect) {
      // execute SQL
      connect.executeSql(text, array, function fnSucessCall(a, b) { 
        success.call(this, Object.values(b.rows)) }.bind(this));
    }.bind(this), function fnExecuteErro(e) {
      // error
      console.log(e);
      error(e);
    }.bind(this));
  ...
  this.cronapi.internal.removeAccents = function (value) {
    let withAccents = 'áàãâäéèêëíìîïóòõôöúùûüçÁÀÃÂÄÉÈÊËÍÌÎÏÓÒÕÖÔÚÙÛÜÇ';
    let withoutAccents = 'aaaaaeeeeiiiiooooouuuucAAAAAEEEEIIIIOOOOOUUUUC';
    let newValue = '';
  ...
  this.cronapi.internal.arrayRemove = function (array, value) {
    let i = this.cronapi.internal.arrayIndexOf(array, value);
    if (i !== -1) {
      array.splice(i, 1);
    }
  };
  ...
  this.cronapi.internal.getAddressWithHostApp = function (address) {
    let urlWithoutEndSlash = window.hostApp || '';
    ...
    if (address) {
      let addressWithoutStartSlash = address.startsWith('/') ? address.substr(1) : address;
  ...
  this.cronapi.internal.stringToJs = function fnStringToJs(str) {
    return (str + '').replace(/[\\"']/g, '\\$&').replace(/\u0000/g, '\\0');
  };

  this.cronapi.internal.getErrorMessage = function fnGetErrorMessage(data, message) {
    try {
   ...
   * @type internal
   */
  this.cronapi.util.upload = function fnUpload(id, description, filter, maxSize, multiple) {
    this.UploadService.upload({
      'description': description,
      'id': id, 'filter': filter, 'maxSize': maxSize,
      'multiple': multiple, 'scope': this
    });
  };
   ...
   * @arbitraryParams true
   */
  this.cronapi.chart.createChart = function fnCreateChart(
    ...
    let CSS_COLOR_NAMES = ["#FF5C00", "#0E53A7", "#48DD00", "#FFD500", "#7309AA", "#CD0074", "#00AF64", "#BF8230", "#F16D95", "#A65000", "#A65000", "#AF66D5"];
    let colorIndex = 0;
    ...
    function getDataset(args) {
      ...
      for (size; size < args.length; size += 1) {
        if (args[size].label) {
   ...
   * @returns {ObjectType.LIST}
   */
  this.cronapi.chart.createDataset = (function fnCreateDataset(
    /** @type {ObjectType.STRING} @description {{createSerieParamName}} */ name,
    /** @type {ObjectType.LIST}  @description {{createSerieParamData}} */ data,
    /** @type {ObjectType.LIST}  @description {{createSerieParamOptions}} */ options){
    const dataset = {};
    if (name) dataset.label = name; else dataset.label = '';
    if (Array.isArray(data)) dataset.data = data;
    else if (data) { dataset.data = JSON.parse(data); } else dataset.data = [];
    if (Array.isArray(options)) {
      dataset.options = options;
    } else dataset.options = JSON.parse(options);
    return dataset;
  })
  ...
  this.cronapi.social.gup = (function fnSocialGup(name, url) {
    if (!url) url = location.href;
    name = name.replace(/[\[]/, "\\\[").replace(/[\]]/, "\\\]");
    const regex = new RegExp('[\\?&]'.concat(name).concat("=([^&#]*)"));
    const results = regex.exec(url);
    return results === null ? null : results[1];
  });

  this.cronapi.social.login = (function fnLogin(login, password, options) {
    let item;
    this.cronapi.screen.showLoading();
    if (!this.cronapi.logic.isNullOrEmpty(this.cronapi.screen.getHostapp())) {
      const jsonLogin = ['{ \"username\": \"', login, '\" , \"password\": \"', password, '\" } '];
      const jsonToken = ['{ \"X-AUTH-TOKEN\": \"', options, '\" } '];
      this.cronapi.util.getURLFromOthers('POST', 'application/x-www-form-urlencoded',
        String(this.cronapi.screen.getHostapp()) + String('auth'),
        this.cronapi.object.createObjectFromString(jsonLogin.join('')),
        this.cronapi.object.createObjectFromString(jsonToken.join('')),
        function fnSenderItemLogged(senderItem) {
          item = senderItem;
        ...
        }.bind(this), function fnSendItem(senderItem){
          item = senderItem;
          if (this.cronapi.object.getProperty(item, 'status') === '403'
            || this.cronapi.object.getProperty(item, 'status') === '401') {
   ...
   * @returns {ObjectType.VOID}
   */
  this.cronapi.social.sociaLogin = (function fnSociaLogin(
    /** @type {ObjectType.STRING} @blockType util_dropdown @keys facebook|github|google|linkedin @values facebook|github|google|linkedin */ socialNetwork,
    /** @type {ObjectType.BOOLEAN} @blockType util_dropdown @keys false|true */ clearCache){
    const that = this;
    const u = window.hostApp + 'signin/'.concat(socialNetwork).concat('/');
    if (cordova.InAppBrowser) {
      let clearCacheString = '';
      if (clearCache === true || clearCache === 'true') {
        clearCacheString = ',clearcache=yes';
      }
      const cref = cordova.InAppBrowser.open(u, '_blank', 'location=no'.concat(clearCacheString));
      cref.addEventListener('loadstart', (function fnAddEventListener(event) {
   ...
   * @returns {ObjectType.OBJECT}
   */
  this.cronapi.screen.getSelectedRowsGrid = (
    function fnScreenSelect(/** @type {ObjectType.STRING} @blockType field_from_screen */field) {
      const result = [];
      const grid = $('[ng-model="'.concat(field, '"]')).children().data('kendoGrid');
      if (grid) {
        const selected = grid.select();
        selected.each(function fnSelectEach() {
