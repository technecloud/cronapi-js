...
public class Operations {
  ...
  public static final Var getSublistFromNToEnd(Var list, Var indexInitial, Var indexFinal) throws Exception {
    ...
    if(list.getType() == Var.Type.LIST) {
      ...
      Var result = new Var(list.getObjectAsList().subList(start, list.getObjectAsList().size() - end));
      return result;
    }
    return Var.newList();
  }
  ...
  public static final Var getSublistFromEndToEnd(Var list, Var indexInitial, Var indexFinal) throws Exception {
    list = isNull(list);
    if(list.getType() == Var.Type.LIST) {
      int start = getStartIndex(list, Var.valueOf((list.size() - indexInitial.getObjectAsInt()) + 1)).getObjectAsInt();
      int end = getEndIndex(list, Var.valueOf((list.size() - indexFinal.getObjectAsInt()) + 1)).getObjectAsInt();
